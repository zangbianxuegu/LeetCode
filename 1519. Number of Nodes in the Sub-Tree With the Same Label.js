...
// https://leetcode.com/problems/number-of-nodes-in-the-sub-tree-with-the-same-label/discuss/743351/Simple-Python-3-Solution-DFS
/**
 * @param {number} n
 * @param {number[][]} edges
 * @param {string} labels
  ...
  function dfs(node) {
    ...
    if (!seen.has(node)) {
      ...
      for (const child of g[node]) {
        for (const [key, value] of Object.entries(dfs(child))) {
          cnt[key] += value
        }
      }
